...
def generate_radiograph_level_reconstructed_and_residue_result(images_tensor, reconstruction_net,
    ...
    while end_row_idx < height:
        ...
        while end_column_idx < width:
            ...
            patch_label_np = pixel_level_label_np[start_row_idx:end_row_idx,
            ...
            reconstructed_patch_tensor, residue_patch_tensor = reconstruction_net(
                patch_image_tensor)
            ...
            counting_mask_patch = counting_mask[start_row_idx:end_row_idx,
                                                start_column_idx:end_column_idx]
            ...
            counting_mask[start_row_idx:end_row_idx,
                          start_column_idx:end_column_idx] = counting_mask_patch
...
def post_process_residue_radiograph(residue_radiograph_np, pixel_level_label_np, prob_threshold, area_threshold):
    ...
    processed_residue_radiograph_np[processed_residue_radiograph_np <
                                    prob_threshold] = 0
    residue_mask_radiograph_np[processed_residue_radiograph_np >=
                               prob_threshold] = 1
...
def generate_micro_calcification_score_list(images_tensor, classification_net, pixel_level_label_np, residue_radiograph_np,
    ...
    for prop in props:
        ...
        positive_prob = classification_preds_tensor.cpu().detach().numpy().squeeze()[
            1]
        ...
        patchDir = "/data/lars/results/patch"
        ...
        patchFolderDir = os.path.join(patchFolderDir, "label")
        if not os.path.exists(patchFolderDir):
            os.mkdir(patchFolderDir)

        filename = filenames[0]

        connected_idx_name = str(connected_idx)

        if connected_idx < 10:
            connected_idx_name = '0' + str(connected_idx)

        rootName = image_Number+'_'+connected_idx_name

        cv2.imwrite(os.path.join(patchFolderDir, rootName+'_patch_image.png'),
                    patch_image_np)

        cv2.imwrite(os.path.join(patchFolderDir, rootName+'_patch_pixel_level_label.png'),
                    patch_pixel_level_label_np)

        cv2.imwrite(os.path.join(patchFolderDir, rootName+'_patch_raw_residue.png'),
                    patch_residue_radiograph_np)

        cv2.imwrite(os.path.join(patchFolderDir, rootName+'_patch_post_processed_residue.png'),
        ...
        sitk.WriteImage(stacked_image, os.path.join(patchFolderDir, rootName +
                                                    '.nii'))
        ...
        cv2.imwrite(os.path.join(patchFolderDir, rootName +
                                 '_prob.png'), probImage)
        ...
        cv2.putText(scoreImage, str(positive_prob * residue_mean),
                    (0, 100), font, 2, (255, 255, 255), 7)
        cv2.imwrite(os.path.join(patchFolderDir, rootName +
                                 '_score.png'), scoreImage)

    return micro_calcification_coordinate_list, micro_calcification_score_list


def label_2_coord_list(images_tensor, classification_net, pixel_level_label_np, residue_radiograph_np,
                       processed_residue_radiograph_np, filenames, prediction_saving_dir, patch_size):
    ...
    if label_num > 0:
        for idx in range(label_num):
            ...
            connected_idx = idx + 1
            indexes = label_connected_components == connected_idx
            centroid_row_idx = label_props[idx].centroid[0]
            centroid_column_idx = label_props[idx].centroid[1]
            ...
            label_image_tensor = images_tensor[:, :,
                                               start_row_idx:end_row_idx, start_column_idx:end_column_idx]

            label_pixel_level_label_np = pixel_level_label_np[start_row_idx:end_row_idx,
                                                              start_column_idx:end_column_idx]

            label_residue_radiograph_np = residue_radiograph_np[start_row_idx:end_row_idx,
                                                                start_column_idx:end_column_idx]

            label_processed_residue_radiograph_np = processed_residue_radiograph_np[
                start_row_idx:end_row_idx, start_column_idx:end_column_idx]

            label_image_np = label_image_tensor.cpu().detach().numpy().squeeze()

            label_image_np *= 255
            label_residue_radiograph_np *= 255
            label_processed_residue_radiograph_np *= 255

            label_pixel_level_label_np[label_pixel_level_label_np == 1] = 255
            label_pixel_level_label_np[label_pixel_level_label_np == 2] = 165
            label_pixel_level_label_np[label_pixel_level_label_np == 3] = 85

            label_image_np = label_image_np.astype(np.uint8)
            label_residue_radiograph_np = label_residue_radiograph_np.astype(
                np.uint8)
            label_processed_residue_radiograph_np = label_processed_residue_radiograph_np.astype(
                np.uint8)
            ...
            labelDir = "/data/lars/results/patch"
            if not os.path.exists(labelDir):
                os.mkdir(labelDir)

            labelFolderDir = os.path.join(labelDir, image_Number)
            if not os.path.exists(labelFolderDir):
                os.mkdir(labelFolderDir)

            labelFolderDir = os.path.join(labelFolderDir, "label")
            if not os.path.exists(labelFolderDir):
                os.mkdir(labelFolderDir)
            ...
            cv2.imwrite(os.path.join(labelFolderDir, rootName+'_label_image.png'),
                        label_image_np)

            cv2.imwrite(os.path.join(labelFolderDir, rootName+'_label_pixel_level_label.png'),
                        label_pixel_level_label_np)

            cv2.imwrite(os.path.join(labelFolderDir, rootName+'_label_raw_residue.png'),
                        label_residue_radiograph_np)

            cv2.imwrite(os.path.join(labelFolderDir, rootName+'_label_post_processed_residue.png'),
                        label_processed_residue_radiograph_np)

            stacked_np = np.concatenate((np.expand_dims(label_image_np, axis=0),
                                         np.expand_dims(
                                             label_pixel_level_label_np, axis=0),
                                         np.expand_dims(
                                             label_residue_radiograph_np, axis=0),
                                         np.expand_dims(
                                             label_processed_residue_radiograph_np, axis=0)), axis=0)

            stacked_image = sitk.GetImageFromArray(stacked_np)
            sitk.WriteImage(stacked_image, os.path.join(labelFolderDir, rootName +
                                                        '.nii'))
            ...
            cv2.putText(probImage, str(positive_prob), (0, 100),
                        font, 2, (255, 255, 255), 7)
            cv2.imwrite(os.path.join(labelFolderDir, rootName +
                                     '_prob.png'), probImage)
            cv2.putText(meanImage, str(residue_mean),
                        (0, 100), font, 2, (255, 255, 255), 7)
            cv2.imwrite(os.path.join(labelFolderDir, rootName +
...
def save_tensor_in_png_and_nii_format(images_tensor, pixel_level_label_np, residue_radiograph_np,
    ...
    sitk.WriteImage(stacked_image, os.path.join(
        prediction_saving_dir, filename.replace('png', 'nii')))
    ...
    processed_residue_radiograph_np = processed_residue_radiograph_np.astype(
        np.uint8)

    cv2.imwrite(os.path.join(prediction_saving_dir, filename.replace('.png', '_patch_image.png')),
                image_np)
    cv2.imwrite(os.path.join(prediction_saving_dir, filename.replace('.png', '_patch_pixel_level_label.png')),
                pixel_level_label_np)
    cv2.imwrite(os.path.join(prediction_saving_dir, filename.replace('.png', '_patch_raw_residue.png')),
                residue_radiograph_np)
    cv2.imwrite(os.path.join(prediction_saving_dir, filename.replace('.png', '_patch_post_processed_residue.png')),
...
def TestMicroCalcificationRadiographLevelDetection(args):
    ...
    visualization_saving_dir = os.path.join(
        args.prediction_saving_dir, 'qualitative_results')
    ...
    reconstruction_net = VNet2d(
        num_in_channels=r_cfg.net.in_channels, num_out_channels=r_cfg.net.out_channels)
    #
    ...
    data_loader = DataLoader(
        dataset, batch_size=1, shuffle=False, num_workers=r_cfg.train.num_threads)

    # set up metrics object
    metrics = MetricsRadiographLevelDetection(
        args.distance_threshold, args.score_threshold_stride)
    ...
    for radiograph_idx, (
        ...
        label_coord_list = label_2_coord_list(images_tensor,
                                              classification_net,
                                              pixel_level_label_np,
                                              residue_radiograph_np,
                                              processed_residue_radiograph_np,
