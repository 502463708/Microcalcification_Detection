...
save_path = r'C:\Users\75209\Desktop\Inbreat_Image_splitted_10_10'
...
def crop_image(image, label, crop_size=10, threshold=1500):
    ...
    for i in range(crop_size):
        ...
        if image[-1, :].sum() < threshold:
            ...
            label = label[:image.shape[0] - 1, :]
        if image[:, 0].sum() < threshold:
            image = image[:, 1:]
            label = label[:, 1:]
        if image[:, -1].sum() < threshold:
            image = image[:, :image.shape[1] - 1]
            label = label[:, :image.shape[1] - 1]
    return image, label
...
def saveimg(name_list=image_train, mode='training'):
    for idx in range(len(name_list)):
        ...
        img, label = crop_image(img, label, threshold=1500)
...
if __name__ == '__main__':
    ...
    # for mode in ['training', 'validation', 'test']:
    #     my_dir = os.path.join(r'C:\Users\75209\Desktop\Inbreat_Image_splitted_10_10', mode)
    #     img_dir = os.path.join(my_dir, 'image')
    #     lab_dir = os.path.join(my_dir, 'labels')
    #     crop_process(img_dir, lab_dir, crop_size=100)
