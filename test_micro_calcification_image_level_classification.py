...
model_saving_dir = '/data/lars/models/20190905_uCs_image_level_classification_CE_default_debug/'
epoch_idx = 50
...
if __name__ == '__main__':
    ...
    for batch_idx, (images_tensor, pixel_level_labels_tensor, _, image_level_labels_tensor, filenames) in enumerate(
        ...
        for patch_idx in range(images_tensor.shape[0]):
            ...
            saving_dir_of_this_patch = os.path.join(prediction_saving_dir, flag_2_dir_mapping[classification_flag])
            ...
            print('consuming time: {:.4f}s'.format(time() - start_time_for_epoch))
            print('---------------------------------------------------------------------------------------------------')